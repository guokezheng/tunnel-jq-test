...
public class CommonUtil {
	 ...
	 * @return
	 */
	public static List<Map<String, Object>> getHostAndPort() {
		List list = new ArrayList<>();
		Map map = new HashMap<>();
		try {
			Properties pro = new Properties();
			pro.load(Thread.currentThread().getContextClassLoader().getResourceAsStream("application-dev.properties"));
			String host = pro.getProperty("issp.alarmPush.host");
			String port = pro.getProperty("issp.alarmPush.port");
			map.put("host", host);
			map.put("port", port);
			list.add(map);
			return list;

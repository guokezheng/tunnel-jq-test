package com.tunnel.deal.plc.omron.util;


import com.tunnel.deal.plc.omron.OmronFinsModel;
import com.tunnel.deal.plc.omron.exception.I18n;
...
public final class OmronUtils {
     ...
     * @return 解析后的结果地址对象
     */
    public static byte[] analysisAddress(String address, boolean isBit) throws PlcException {
        ...
        try {
            switch (address.charAt(0)) {
                ...
                case 'e':
                    ...
                    break;
                default: throw new PlcException("不支持的地址["+address+"]", "欧姆龙");
            }
            ...
            if (address.charAt(0) == 'E' || address.charAt(0) == 'e') {
                ...
                if (isBit) {
                    ...
                    if (splits.length > 2) {
                        ...
                        if (result[3] > 15) {
                            throw new PlcException("输入的位地址只能在0-15之间", "欧姆龙");
                        }
                    }
                } else {
                    // 字操作
            ...
            throw new PlcException("地址格式错误["+address+"]", "欧姆龙");
        }
        return result;
    }
     ...
     * @return
     */
    public static byte[] responseValidAnalysis(byte[] response, boolean isRead) throws PlcException {
    ...
    public static int getErrorCode(byte[] response) throws PlcException {
        ...
        throw new PlcException("数据接收异常", "欧姆龙");
    }
     ...
     * @return
     */
    public static boolean isSuccess(byte[] response) throws PlcException {
    ...
    public static void validErrorCode(byte[] response) throws PlcException {
        ...
        if(errorCode > 0) {
            throw new PlcException(getStatusDescription(errorCode), "欧姆龙");
        }
    }
     ...
     * @return 带有是否成功的结果对象
     */
    public static byte[] udpResponseValidAnalysis(byte[] response, boolean isRead) throws PlcException {
        if (response.length >= 14) {
            ...
            if (!isRead) {
                ...
                if (content.length > 0) {
                    ...
                    throw new PlcException(getStatusDescription(err), "欧姆龙");
                }

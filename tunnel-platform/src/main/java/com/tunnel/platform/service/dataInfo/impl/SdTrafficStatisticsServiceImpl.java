...
import com.tunnel.platform.mapper.dataInfo.SdTrafficStatisticsMapper;
		...
		if("month".equals(type)){
			for(int i=1;i<=12;i++){
				...
				for(int j=0;j<data.size();j++){
					...
					String strDate = date.substring(date.length()-2, date.length());
			...
			if(data.size() > 0){
				...
				for(int i=1;i<=days;i++){
					...
					for(int j=0;j<data.size();j++){
						...
						String strDate = date1.substring(date1.length()-2, date1.length());
						if(iDate.equals(strDate)){
							flag = true;
						}
					}

...
//                logger.info(hostAddress + "设备类型" + devType + "返回内容" + hexProtocol);
                if (nIndex <= commands.size()) {
                    if (devType != null) {
                        DataUtil.parsingCIOContent(hostId, devType, hexProtocol);
                    }
                    if (nIndex < commands.size()) {
                        ...
                        for (Map.Entry<String, String> entry : strCom.entrySet()) {
                            ...
                            if (devType == "kz") {
                                DataUtil.parsingKZ(hostId, hexProtocol, commands.get(nIndex));
...
//                logger.info(hostAddress + "设备类型" + devType + "返回内容" + hexProtocol);
                if (nIndex <= commands.size()) {
                    if (devType != null) {
                        DataUtil.parsingDMContent(devType, hexProtocol);
                    }
                    if (nIndex < commands.size()) {
                        Map<String, String> strCom = commands.get(nIndex);
                        for (Map.Entry<String, String> entry : strCom.entrySet()) {

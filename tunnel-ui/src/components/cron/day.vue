  ...
  <div :val="value_">
    ...
    <div>
      ...
      <el-checkbox-group v-model="appoint">
        <div  v-for="i in 4" :key="i" style="margin-left: 15px;  line-height: 15px;">
          <el-checkbox @change="type = '4'"  v-for="j in 10" v-if="parseInt((i - 1) + '' + (j - 1)) < 32 && !(i === 1 && j === 1)" :key="j" :label="(i - 1) + '' + (j - 1)" style="width: 20px;"></el-checkbox>
        </div>
      </el-checkbox-group>
    </div>
  </div>

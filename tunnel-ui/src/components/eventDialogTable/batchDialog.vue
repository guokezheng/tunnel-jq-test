<template>
  <div>
    <el-dialog title="批量执行" :visible.sync="batchDialog" :before-close="cancel" width="40%" text-align="left"
      class="detailsDialog" :close-on-click-modal="false" :modal="false" append-to-body>
      <div class="dialogStyleBox">
        <div class="dialogLine"></div>
        <div class="dialogCloseButton"></div>
      </div>
      <el-form :model="form" ref="form" label-width="80px" class="dialogForm" :rules="rules">
        <el-row>
          <el-col :span="12">
            <el-form-item label="影响方向" prop="direction">
              <el-select v-model="form.direction" placeholder="方向" clearable size="small" style="width: 100%">
                <el-option v-for="item in directionList" :key="item.dictValue" :label="item.dictLabel"
          ...
          <el-col :span="12">
            <el-form-item label="影响车道">
              <el-select v-model="form.laneNo" placeholder="车道" clearable size="small" multiple collapse-tags
                style="width: 100%">
                <el-option v-for="(item, index) in chezhiLaneList" :key="index" :label="item.dictLabel"
                  :value="item.dictValue" />
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="预估类型" prop="eventTypeId">
              <el-select v-model="form.eventTypeId" clearable size="small" style="width: 100%"
                @change="getReservePlanData">
                <el-option v-for="(item, index) in eventTypeData" :key="index" :label="item.simplifyName"
                  :value="item.id" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="预估等级" prop="eventGrade">
              <el-select v-model="form.eventGrade" clearable size="small" style="width: calc(100% - 10px)"
                @change="getReservePlanData">
        ...
        <el-form-item v-show="eventIsShow(form.reviewRemark)" prop="otherContent">
          <el-input placeholder="请输入其他原因内容" v-model="form.otherContent"></el-input>
        </el-form-item>

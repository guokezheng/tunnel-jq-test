  ...
  <div v-if="!item.hidden">
    <template v-if="hasOneShowingChild(item.children,item) && (!onlyOneChild.children||onlyOneChild.noShowingChildren)&&!item.alwaysShow">
      <app-link style="height:100%;" v-if="onlyOneChild.meta" :to="resolvePath(onlyOneChild.path, onlyOneChild.query)">
        <el-menu-item :id="topNav?'':'left_nav'" :style="topNav?'color:white;vertical-align: bottom;display:flex;height:65px;line-height:65px;justify-content: left;align-items: center;':'display:flex;align-items: center;height:65px;line-height:65px;padding-left: 12px;'" :index="resolvePath(onlyOneChild.path)"
          :class="{'submenu-title-noDropdown':!isNest}">
          <!-- <item :icon="onlyOneChild.meta.icon||(item.meta&&item.meta.icon)" :title="onlyOneChild.meta.title" /> -->
          <item :title="onlyOneChild.meta.title" style="color: white;"/>
        </el-menu-item>
      </app-link>
    </template>
...
<style lang="scss" scope>
  .el-submenu__title{height:65px;line-height:65px;}

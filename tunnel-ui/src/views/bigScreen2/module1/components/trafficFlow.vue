<template>
  <div class="tabBox">
    <el-radio-group v-model="tabModel" class="tabButton">
      ...
      <el-radio-button label="各隧道车流量"></el-radio-button>
    </el-radio-group>
    <div class="row2" v-show="tabModel == '总车流量'">
      <div>今日总车流量</div>
      <div>
        <span class="yellow">{{ dayCarNum }}</span><span>辆</span>
      </div>
      <!-- <div>
    ...
    <div id="top10" v-show="tabModel == '各隧道车流量'" ref="top10"></div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      getList() {
        carFlow().then((res) => {
          console.log(res, "res")
          this.dayCarNum = res.data.dayCarNum;
        ...
        let tunnelTop = res.data.tunnelTop;
        let xData1 = res.data.xData;
        let yData1 = res.data.yData;
        let xData2 = [];
        let yData2 = []
        for (let i = 0; i < tunnelTop.length; i++) {
          xData2.push(tunnelTop[i].tunnelName);
          yData2.push(tunnelTop[i].carCount);
        }
        // console.log(xData2,yData2,"tunnelTop")
        // let xData2 = [0, 1, 2, 3, 4, 5]
        // let yData2 = [10, 11, 12, 13, 14, 15]
        // let xData2 = ["金家楼隧道"]
        // let yData2 = [5434]

        if (this.tabModel == "总车流量") {
          this.$nextTick(()=>{
            this.initEnergyConsumption(xData1, yData1);
          })
        } else {
          this.$nextTick(()=>{
            this.initEchats2(xData2, yData2);
          })
        }
        });
      },
      ...
      // getEnergyConsumption( fourHoursList) {
      ...
      // },
      // 能耗监测echarts
      initEnergyConsumption(xData, yData) {
        this.$nextTick(function () {
          if (
  ...
  .tabBox {
    ...
    ::v-deep .el-radio-group .el-radio-button__inner {
      background: linear-gradient(180deg,
          ...
          rgba($color: #0079db, $alpha: 0.8)) !important;
      border: none;
      color: #fff;
    }

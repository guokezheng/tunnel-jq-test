<template>
  <div class="app-container">
    ...
    <el-table 
      ...
      <el-table-column label="隧道名称" align="center" prop="tunnelId" :formatter="gettunnelName"/>
      <el-table-column label="模式类型" align="center" prop="modeType" :formatter="getModeType"/>
      <!-- <el-table-column label="时间段配置参数" align="center" prop="timeSlot" /> -->
      <el-table-column label="当前亮度值" align="center" prop="beforeLuminance" />
      <el-table-column label="最大车流量(辆)" align="center" prop="maxTrafficFlow" />
      <el-table-column label="最小亮度值" align="center" prop="minLuminance" />
      <el-table-column label="响应时长(毫秒)" align="center" prop="respondTime" />
      <el-table-column label="调光最大区间" align="center" prop="maxLuminanceRange" />
      <el-table-column label="调光最小区间" align="center" prop="minLuminanceRange" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="30%" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="当前亮度值" prop="beforeLuminance" v-if="form.modeType == 1">
              <el-input v-model="form.beforeLuminance" placeholder="请输入当前亮度值(0-100)" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="最大车流量(辆)" prop="maxTrafficFlow">
              <el-input v-model="form.maxTrafficFlow" placeholder="请输入最大车流量" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="响应时长(毫秒)" prop="respondTime" v-if="form.modeType != 0">
              <el-input v-model="form.respondTime" placeholder="请输入响应时长 单位：秒s" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="调光最大区间" prop="maxLuminanceRange">
              <el-input v-model="form.maxLuminanceRange" placeholder="请输入调光最大区间" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="调光最小区间" prop="minLuminanceRange">
              <el-input v-model="form.minLuminanceRange" placeholder="请输入调光最小区间" />
            </el-form-item>
          </el-col>
        </el-row>
        <div v-if="form.modeType != 1">
          ...
          ><el-row :gutter="15">
              ...
              <el-col :span="2">
                <el-button type="primary" @click="addTimeSlot" v-if="index!=(timeSlotList.length-1)||index == 0">+</el-button>
                <el-button type="danger" icon="el-icon-delete" @click="delTimeSlot(item)"  v-if="index == (timeSlotList.length-1)&&index != 0"></el-button>
              </el-col>
            </el-row>
          </el-form-item>
        </div>
...
<script>
import { listConfig, getConfig, delConfig, addConfig, updateConfig, exportConfig } from "@/api/business/enhancedLighting/app.js"
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getConfig(id).then(response => {
        ...
        for (let index = 0; index < this.timeSlotList.length; index++) {
          ...
          param.startTime = new Date(("1970-01-01 "+param.startTime).replace(/-/g, "/"))
          param.endTime = new Date(("1970-01-01 "+param.endTime).replace(/-/g, "/"))
        }
    ...
    submitForm() {
      if(this.form.modeType != 1){
        ...
        for (let index = 0; index < this.timeSlotList.length; index++) {
          const param = this.timeSlotList[index];
          param.startTime = (param.startTime.getHours()<10?"0"+param.startTime.getHours():param.startTime.getHours())+":"+(param.startTime.getMinutes()<10?"0"+param.startTime.getMinutes():param.startTime.getMinutes())+":"+(param.startTime.getSeconds()<10?"0"+param.startTime.getSeconds():param.startTime.getSeconds());
          param.endTime = (param.endTime.getHours()<10?"0"+param.endTime.getHours():param.endTime.getHours())+":"+(param.endTime.getMinutes()<10?"0"+param.endTime.getMinutes():param.endTime.getMinutes())+":"+(param.endTime.getSeconds()<10?"0"+param.endTime.getSeconds():param.endTime.getSeconds());
        }
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除加强照明配置信息编号为"' + ids + '"的数据项？').then(function() {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有加强照明配置信息？').then(() => {

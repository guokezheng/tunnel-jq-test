...
<template>
  <div class='department'>
    <treeselect class="tree_sty" @select="checkChange" v-model="deptId" :options="deptOptions" :show-count="true" placeholder="请选择归属部门" :disable-branch-nodes="true" :beforeClearAll="clearable" />
  </div>
</template>
<script>
  ...
  methods: {
    ...
    getTreeselect() {
      treeselect().then(response => {
        ...
        this.$emit('getTree',this.firstSelectItem[0].id )
      })
    },
    // 单选回路
    checkChange(data) {

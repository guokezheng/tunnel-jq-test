...
<template>
  <div class='department'>
    <treeselect style="width: 100%;font-size: 13px;" @select="checkChange" v-model="deptId" :options="deptOptions" :show-count="true" placeholder="请选择归属部门" :disable-branch-nodes="true" :beforeClearAll="clearable"/>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    getTreeselect() {
      treeselectExcYG1().then(response => {
        ...
        this.deptId = this.firstSelectItem[0].id
        this.$emit('getTreeFirst', this.firstSelectItem[0].code)
      })
    },
    // 单选回路
    checkChange(data) {

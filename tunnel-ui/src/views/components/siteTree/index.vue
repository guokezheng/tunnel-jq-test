...
<template>
  <div>
    <div style="
        ...
        " class="box">
      <el-row v-if="filter">
        <el-input v-model="label" placeholder="请输入机构名称" clearable size="small" suffix-icon="el-icon-search" />
      </el-row>
    </div>
    <el-scrollbar>
      <el-row class="leftTree">
        ...
        <el-tree class="tree" :data="siteTreeOptions" :props="defaultProps" :expand-on-click-node="false" :check-on-click-node="true" :show-checkbox="show_checkbox" :check-strictly="!check_strictly" :filter-node-method="filterNode" ref="tree" default-expand-all @node-click="handleNodeClick" @check="handleCheckChange" node-key="code" highlight-current>
          <!-- 鼠标移入文本提示 -->

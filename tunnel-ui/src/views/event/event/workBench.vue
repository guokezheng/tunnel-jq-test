<template>
  <div style="width: 100%; height: 100%">
    <el-dialog v-dialogDrag class="workbench-dialog dispatchWorkBench" title="工作台" width="83%" append-to-body
      ...
      <div class="header">
        ...
        <div class="headerRight">
          <div class="display-box zoomClass" ref="treeBox">
            <el-input placeholder="请输入内容" v-model="screenEqName" class="input-with-select" clearable
              @click.native="treeClick()" @keyup.enter.native="screenEqNameButton()" @clear="treeClear">
            ...
            <div class="treeBox" ref="treeBox" v-show="treeShow" :style="dragFlag ? '47%' : '54.5%'">
              <el-tree :show-checkbox="false" :data="treeData" :props="defaultProps" @node-click="handleNodeClick"
                :filter-node-method="filterNode" accordion ref="tree" node-key="id"></el-tree>
            </div>
          </div>
          ...
          <el-button v-if="resetCanvasFlag" class="buttons" type="primary" size="mini" icon="el-icon-map-location"
            @click="resetCanvas">
            地图复位
          </el-button>
          <el-button class="buttons" type="primary" size="mini" @click="batchManage" v-if="batchManageType == 1">
          ...
          <el-button class="buttons" type="primary" size="mini" @click="iconExplain">
            <img src="../../../assets/icons/tbhy.png" />
            <span>图标含义</span>
          </el-button>
          <div class="buttonsDeawer" @click="isDrawer()">
            <img src="../../../assets/icons/deawer.png" :style="{
      ...
      <div class="footer">
        ...
        <div class="vehicleLane" @mouseover="mouseoversImage" @mouseleave="mouseleaveImage">
          <div class="content" ref="divRoller" @wheel.prevent="handleTableWheel" @mousewheel.prevent="mouseSrollAuto"
            @contextmenu.prevent @mousedown="dragImg">
            <div class="workbench-content" ref="dragImgDom">
              <!--画布区域-->
              <div>
                <el-row class="config-img-area" id="config-img-id"
                  :style="{ width: currentTunnel.lane.width / 1.2 + 'px' }">
                  <el-image class="back-img" :src="currentTunnel.lane.url"
                    :style="{ width: currentTunnel.lane.width / 1.2 + 'px' }"></el-image>
                  <div class="wrapper" id="eq-wrapper" ref="eqWrapper">
                    ...
                    <template>
                      <div style="
                            ...
                            left: 178px;
                          " v-show="robotShow">
                        <img src="../../../assets/logo/equipment_log/机器人-在线.png" class="robotAnimation" :style="{
                              right: robotPositon + '%',
                            }" @click="clickRobot()" />
                      </div>
                    </template>
                    <div class="icon-box mouseHover" v-for="(item, index) in selectedIconList" :key="item.eqId" :style="{
                        ...
                        item.eqType == 7 || item.eqType == 8 || item.eqType == 9
                          ...
                          : ''
                      " @click="openStateSwitch(item)" @mousemove="openTooltip(item, index)"
                      @mouseleave="closeTooltip(item)">
                      <div class="tooltipBox" v-if="showTooltipIndex == index" :style="{
                          top:
                        ...
                        <img v-show="
                          ...
                          :key="item.eqId + indexs" :src="url" :class="
                            item.eqName == screenEqName
                              ...
                              : ''
                          " />
                        <img v-show="item.eqType == '31'" style="position: absolute" :style="{
                            cursor:
                              item.eqType || item.eqType == 0 ? 'pointer' : '',
                            border:
                              item.click == true ? 'solid 2px #09C3FC' : '',
                            ...
                            item.eqName == screenEqName
                              ...
                              : ''
                          " />
                        <div v-show="item.eqType == 16" class="boardBox1" :style="{
                            cursor:
                              item.eqType || item.eqType == 0 ? 'pointer' : '',
                            border:
                              item.click == true ? 'solid 2px #09C3FC' : '',
                          ...
                          <div :style="{
                            ...
                            <span v-for="itm in getBoardStyle(
                                item.associated_device_id,
                                'array'
      ...
      <el-drawer title="一键控制" :visible.sync="buttonsDeawer" :modal="false" :append-to-body="true" class="drawerBox">
        <el-row>
          <el-col :span="12">
            <div class="drawerItem zoomClass">
              ...
              <el-switch v-model="srollSwitch" class="switchStyle" @change="srollSwitchChange"></el-switch>
            </div>
          </el-col>
          <!-- <el-col :span="12">
            <div class="drawerItem zoomClass">
    ...
    <el-dialog v-dialogDrag class="workbench-dialog explain-table icon-dialog" :title="title"
      ...
      <img src="@/assets/logo/equipment_log/all.png" style="width: 100%; height: auto" />
    ...
    <el-dialog class="workbench-dialog vehicle-dialog" :title="title" :visible.sync="batchManageDialog" width="450px"
      append-to-body v-dialogDrag :close-on-click-modal="false" :model="false">
      <div class="dialogStyleBox">
        <div class="dialogLine"></div>
        <div class="dialogCloseButton"></div>
      </div>
      <el-table ref="batchManageTable" :data="batchManageList" tooltip-effect="dark"
        style="width: 100%; margin-bottom: 10px !important" max-height="220" size="mini">
        <el-table-column prop="eqName" label="设备名称" width="200" align="center" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="pile" label="桩号" width="120" align="center" show-overflow-tooltip>
        </el-table-column>
      ...
      <el-form :model="batchManageForm" label-width="90px" label-position="left" size="mini"
        style="padding: 15px; padding-top: 0px">
        <el-form-item label="配置状态:">
          <div class="wrap">
            <el-radio-group v-for="item in eqTypeStateList2" :key="item.state" v-model="batchManageForm.state"
              style="display: flex; flex-direction: column" @change="$forceUpdate()">
              <el-radio v-if="batchManageForm.eqType == item.type && item.control == 1" class="el-radio flex-row"
                ...
                <el-row class="flex-row" v-if="
                    ...
                    (batchManageForm.eqType == 1 || batchManageForm.eqType == 2)
                  ">
                  <img :width="iconWidth" :height="iconHeight" :src="item.url[1]" />
                  <img :width="iconWidth" :height="iconHeight" :src="item.url[0]" v-if="item.url.length > 1" />
                  <div style="margin: 0 0 0 10px; display: inline-block">
                    {{ item.name }}
                  </div>
                </el-row>
        ...
        <el-row style="margin-top: 10px" v-show="[7, 9, 30, 31].includes(batchManageForm.eqType)">
          <el-col :span="15">
            <el-form-item label="亮度调整:">
              <el-slider v-model="batchManageForm.brightness" :max="100" :min="min" class="sliderClass"
                :disabled="!batchManageForm.brightness" @change="$forceUpdate()"></el-slider>
            </el-form-item>
          </el-col>
          <el-col :span="9">
            <span style="padding-left: 10px; line-height: 30px">{{ batchManageForm.brightness }} %</span>
          </el-col>
        </el-row>
        <el-row style="margin-top: 10px" v-show="
            [30, 31].includes(batchManageForm.eqType) &&
            batchManageForm.state == 5
          ">
          <el-col :span="15">
            <el-form-item label="闪烁频率:">
              <el-slider v-model="batchManageForm.frequency" :max="100" :min="min" class="sliderClass"
                :disabled="!batchManageForm.frequency" @change="$forceUpdate()"></el-slider>
            </el-form-item>
          </el-col>
          <el-col :span="9">
            <span style="padding-left: 10px; line-height: 30px">{{ batchManageForm.frequency }} m/s</span>
          </el-col>
        </el-row>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="batchManageOK()" class="submitButton">确 定</el-button>
        <el-button class="closeButton" @click="closeBatchManageDialog()">取 消</el-button>
      </div>
    </el-dialog>
  </div>
</template>

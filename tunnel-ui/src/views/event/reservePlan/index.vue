...
<template>
  <div class="app-container">
    ...
    <el-row :gutter="20" class="topFormRow">
      <el-col :span="4">
        <el-button v-hasPermi="['business:plan:add']" size="small" @click="handleAdd()">新增
        </el-button>
      ...
      <el-col :span="6" :offset="14">
        <div class="grid-content bg-purple" ref="main">
          <el-input placeholder="请输入预案名称，回车搜索" v-model="queryParams.planName" @keyup.enter.native="handleQuery"
            size="small">
    ...
    <div class="searchBox" v-show="boxShow" ref="cc">
      <el-form ref="queryForm" :inline="true" :model="queryParams" label-width="75px">
        ...
        <el-form-item label="所属隧道" prop="tunnelId" v-show="manageStatin == '0'">
          <el-select v-model="queryParams.tunnelId" placeholder="请选择所属隧道" @change="changeSelection" clearable
            size="small">
            <el-option v-for="(item, index) in eqTunnelData" :key="index" :label="item.tunnelName"
              :value="item.tunnelId"></el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="方向" prop="direction">
          <el-select v-model="queryParams.direction" placeholder="请选择方向" clearable style="width: 100%"
            @change="$forceUpdate()">
            <el-option v-for="(item, index) in directionData" :key="index" :label="item.dictLabel"
        ...
        <el-form-item label="防控类型" prop="prevControlType">
          <el-select v-model="queryParams.prevControlType" placeholder="请选择防控类型" clearable size="small"
            @change="handleChangeControl(1)">
            <el-option v-for="item in controlTypeOptions" :key="item.dictValue" :label="item.dictLabel"
              :value="item.dictValue" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="事件等级" prop="eventGrade">
          <el-select v-model="queryParams.eventGrade" clearable placeholder="请选择事件等级" size="small">
            <el-option v-for="(item, index) in eventGradeList" :key="index" :label="item.dictLabel"
              :value="item.dictValue" />
          </el-select>
        </el-form-item>
    ...
    <el-table ref="planTable" v-loading="loading" :data="planList" @selection-change="handleSelectionChange"
      @row-click="handleRowClick" height="62vh" class="allTable" :row-key="getRowKey">
      <el-table-column type="selection" width="55" align="center" reserve-selection />
      <el-table-column type="index" width="70" align="center" label="序号">
      </el-table-column>
      <el-table-column align="center" label="隧道名称" prop="sdTunnels.tunnelName" width="130" />
      <el-table-column align="center" label="方向" prop="direction" width="130" :formatter="directionFormat" />
      <el-table-column align="center" label="防控类型" prop="prevControlType" :formatter="prevControlTypeFormat" />
      <el-table-column align="center" label="事件类型" prop="planTypeId">
        <template slot-scope="scope">
          <span>{{ getPlanTypeData(scope.row.planTypeId) }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="事件等级" prop="eventType.eventGrade" :formatter="eventGradeFormat">
      </el-table-column>
      <el-table-column align="center" label="预案名称" prop="planName">
      </el-table-column>
      <el-table-column align="center" label="预案描述" prop="planDescription" width="200" :show-overflow-tooltip="true">
      </el-table-column>

      <el-table-column align="center" class-name="small-padding fixed-width" label="相关文档" width="130">
        <template slot-scope="scope">
          <el-button v-show="scope.row.planFileId && scope.row.planFileId != 'null'" icon="el-icon-link" size="mini"
            style="cursor: pointer; color: #39adff" type="text" @click="openFileDrawer(scope.row)">点击查看
          </el-button>
      ...
      <el-table-column align="center" class-name="small-padding fixed-width" label="操作">
        <template slot-scope="scope">
          <el-button v-hasPermi="['business:plan:edit']" class="tableBlueButtton" size="mini"
            @click="handleUpdate(scope.row)">修改
          </el-button>
          <el-button v-hasPermi="['business:plan:remove']" size="mini" class="tableDelButtton"
            @click="handleDelete(scope.row)">删除
          </el-button>
          <el-button v-hasPermi="['business:plan:add']" size="mini" class="tableBlueButtton"
            @click="chooseStrategyInfo(scope.row)">配置策略
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="drawerFileTitle" :visible.sync="drawerFile" width="500px" append-to-body
      :before-close="handleFileClose" :close-on-click-modal="false" class="explain-table">
    ...
    > -->
      <el-table v-loading="loading" :data="planFileList">
        ...
        <el-table-column align="center" class-name="small-padding fixed-width" label="操作">
          <template slot-scope="scope">
            <el-button v-hasPermi="['business:plan:edit']" :loading="loadFileLoading" size="mini" type="text"
              @click="loadFile(scope.row)">下载
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="title" :visible.sync="handleStrategyVisible" append-to-body width="60%"
      ...
      <div style="width: 100%; height: 31.25rem; overflow: auto">
        <el-table ref="multipleTable" :data="handleStrategyList" empty-text="暂无策略" row-key="id"
    ...
    <el-dialog :title="addStrategyTitle" :visible.sync="addStrategyVisible" append-to-body width="60%"
      ...
      <div style="width: 100%; height: 31.25rem; overflow: auto">
        <el-table ref="addMultipleTable" :data="addStrategyList" empty-text="暂无策略" row-key="id"
    ...
    <!-- 新增弹窗 -->
    <el-dialog :title="title" :destroy-on-close="true" :visible.sync="dialogFormVisible" width="500px"
      :before-close="cancelsubmitUpload" :close-on-click-modal="false">
      <div class="dialogStyleBox">
        <div class="dialogLine"></div>
        <div class="dialogCloseButton"></div>
      </div>
      <el-form ref="addform1" :model="reservePlanDrawForm" :rules="rules" label-width="80px">
        <el-form-item label="所属隧道" prop="tunnelId">
          <el-select v-model="reservePlanDrawForm.tunnelId" placeholder="请选择所属隧道" style="width: 100%"
        ...
        <el-form-item label="隧道方向" prop="direction">
          <el-select v-model="reservePlanDrawForm.direction" placeholder="请选择隧道方向" style="width: 100%">
            <el-option v-for="(item, index) in directionData" :key="index" :label="item.dictLabel"
              :value="item.dictValue"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="防控类型" prop="prevControlType">
          <el-select v-model="reservePlanDrawForm.prevControlType" placeholder="请选择防控类型" clearable size="small"
            style="width: 100%" @change="handleChangeControl(2)">
            <el-option v-for="item in controlTypeOptions" :key="item.dictValue" :label="item.dictLabel"
              :value="item.dictValue" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="预案名称" prop="planName">
          <el-input v-model="reservePlanDrawForm.planName" placeholder="请输入预案名称" style="width: 100%" />
        </el-form-item>
        ...
        <el-form-item label="相关文档" prop="eventLocation">
          <el-upload ref="upload" :auto-upload="false" :file-list="fileList" :http-request="uploadFile" :limit="5"
            :on-change="handleChange" :on-exceed="handleExceed" :on-preview="handlePreview" :on-remove="handleRemove"
            action="http://xxx.xxx.xxx/personality/uploadExcel" class="upload-demo" multiple style="width: 100%">
      ...
      <div class="dialog-footer" slot="footer">
        ...
        <el-button @click="cancelsubmitUpload" class="closeButton">取 消</el-button>
      </div>
    </el-dialog>

    <!-- 配置策略 -->
    <el-dialog :before-close="closeStrategy" title="联控流程" :visible.sync="strategyVisible" append-to-body width="78%"
      class="strategy-dialog" :close-on-click-modal="false">
      <div class="dialogStyleBox">
        <div class="dialogLine"></div>
        <div class="dialogCloseButton"></div>
      </div>
      <el-form ref="form1" :inline="true" :model="reserveProcessDrawForm">
        <div v-for="(item, number) in planTypeIdList" :key="number" class="dialongBox">
          <el-row :gutter="20">
            <el-col :span="8">
              <el-form-item prop="processStageName" label="阶段名称:" label-width="80px">
                <el-input v-model="item.processStageName" placeholder="阶段名称"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="4" :offset="12" style="display: flex; justify-content: flex-end">
              <el-button class="flex-row dialogButton" icon="el-icon-delete" @click="deleteInfo(number)">
                删除
              </el-button>
              <el-button class="flex-row dialogButton" type="primary" size="mini" icon="el-icon-plus"
          ...
          <el-row class="planBox">
            <el-col :span="24" class="colflex" v-for="(itemed, index) in item.processesList" :key="index">
              <el-col :span="1" style="text-align: center; padding-left: 0px !important">
              ...
              <el-col :span="4">
                <el-form-item prop="itemed.processName">
                  <el-input v-model="itemed.processName" placeholder="操作处置名称"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="4">
                <el-cascader v-model="itemed.eqTypeId" :options="equipmentTypeData" :props="equipmentTypeProps"
                  :show-all-levels="false" @change="changeEquipmentType(itemed.eqTypeId, number, index)"
                  style="width: 100%" popper-class="cascaderClass"></el-cascader>
              </el-col>
              <el-col :span="4">
                <el-select v-model="itemed.retrievalRule" placeholder="规则条件" clearable
                  @change="ruleChange(number, index, itemed.retrievalRule)" @visible-change="ruleVisible">
                  <el-option v-for="(tag, inx) in itemed.retrievalRuleList" :key="inx" :label="tag.dictLabel"
                    :value="tag.dictValue" :disabled="tag.disabled" />
                </el-select>
              </el-col>
              <el-col :span="5">
                <!-- @change="qbgChange(number, index, itemed.equipments)" -->
                <el-cascader v-model="itemed.equipments" :options="itemed.equipmentData" :disabled="itemed.disabled"
                  :props="devicesProps" :show-all-levels="false" collapse-tags style="width: 100%" node-key="label"
                  @check-change="treeChange" ref="tree" popper-class="cascaderClass"></el-cascader>
              </el-col>
              ...
              <el-col :span="4" v-if="
                ...
                <el-select v-model="itemed.state" placeholder="设备操作">
                  <el-option v-for="(ite, idx) in itemed.eqStateList" :key="idx" :label="ite.stateName"
              ...
              <el-col :span="2" v-if="
                  ...
                  itemed.state == '1'
                ">
                <el-input-number style="width: 100%" v-model="itemed.brightness" :min="itemed.minLight" :max="100"
                  label="亮度"></el-input-number>
              </el-col>
              <!-- 照明设备end -->
              <!-- 选择情报板模板 -->
              <el-col :span="4" v-if="itemed.eqTypeId == '16' || itemed.eqTypeId == '36'">
                <!-- <el-cascader
                  placeholder="请选择模板"
                  :props="checkStrictly"
                ...
                <el-input v-model="itemed.content" placeholder="请选择模板" readonly @click.native="openTemDialog(itemed)">
                  <el-button slot="append" icon="el-icon-search" @click.stop="templateClick(number, index, itemed)">
                  </el-button>
                </el-input>
              </el-col>
              ...
              <el-col :span="4" v-if="itemed.eqTypeId == '22'">
                <el-form-item prop="itemed.state">
                  <el-select v-model="itemed.state" placeholder="播放文件">
                    <el-option v-for="(itemPl, figure) in itemed.templatesList" :key="figure" :label="itemPl.name"
              ...
              <el-col :span="2" class="dialogTableButtonBox">
                <el-button class="delete" @click="removeItem(number, index)"></el-button>
                <el-button class="add" @click="addItem(number, index)"></el-button>
              </el-col>
            </el-col>
          </el-row>
        </div>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button class="submitButton" v-hasPermi="['plan:process:add']" @click="submitStrategy">保存</el-button>
    ...
    <el-dialog :title="templateData.processName" :visible.sync="dialogVisibleTem" width="45%"

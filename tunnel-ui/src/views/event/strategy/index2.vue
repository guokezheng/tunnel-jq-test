  ...
  <div class="app-container">
        ...
        <template v-if="is_show">
          ...
          <el-row>
            <template v-if="strategyForm.strategyType == '0'">
              <el-col :span="8">
                <el-form-item label="设备类型">
                  ...
                  >
                    <el-option
                      v-for="(item, index) in equipmentTypeData"
                      :key="index"
                      :label="item.typeName"
                      :value="item.typeId"
            ...
            <el-col :span="8">
              <el-form-item label="方向">
                ...
                >
                  <el-option
                    v-for="(dict, index) in directionOptions"
                    :key="index"
                    :label="dict.dictLabel"
                    :value="dict.dictValue"
                    @click.native="resetData"
                    @change.native="eqTypeChange(dict.dictValue)"
                  />
                </el-select>
              </el-form-item>
            </el-col>
          ...
          <template v-if="strategyForm.strategyType == 0">
            ...
            >
              ...
              >
                <el-option
                  v-for="(item, idx) in equipmentData"
                  :key="idx"
                  :label="item.eqName"
                  :value="item.eqId"
                  :disabled="item.disabled"
                  @click.native="manualControlChange($event)"
                />
                <el-option
                  label="暂无数据"
                  disabled
                  value=""
                  v-show="equipmentData.length < 1"
                />
              </el-select>
              <el-select
                v-model="items.state"
          ...
          <template v-if="strategyForm.strategyType != 0">
            ...
            >
              <div
                ...
                "
              >
                <template v-if="strategyForm.strategyType == '3'">
                  <el-form-item style="width: 30%; margin-top: 20px">
                    <el-time-picker
                      v-model="dain.timeControl"
                      :picker-options="{
                ...
                <el-form-item style="width: 65%; margin-top: 20px">
                  <el-input
    ...
    >
      ...
      >
        <el-form-item
          label="设备类型"
          v-if="equipmentTypeData.length"
          :style="{
            height: viewStrategy ? '2.75rem' : '13.75rem',
            overflow: 'auto',
          }"
          prop="equipment_type"
        >
...
export default {
  ...
  data() {
    return {
      ...
      strategyForm: {
        manualControl: [{ value: "", state: "" }], //手动控制数组
        autoControl: [
          {
            value: "",
            state: "",
  ...
  methods: {
    ...
    addCf() {
      ...
      listDevices({
        ...
        let data = res.rows;
        if (this.chooseEq && this.strategyForm.manualControl.length > 1) {
    ...
    openInsertStrategy() {
      ...
      (this.strategyForm = {
        ...
        this.getEquipmentType(); //此时没有隧道id，不用去查设备类别

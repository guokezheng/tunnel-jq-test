...
export default {
  name: 'echartsIndustry',
  data() {
    return {}
  },

  props: ['powerCode', 'GongYeDateValue', 'GongYeValue'],
  // 注册组件
  components: {},
  // 计算属性
  computed: {
  ...
  methods: {
    async getInitECharts() {
      if (this.powerCode || this.GongYeDateValue || this.GongYeValue) {
        let obj = {}
        obj.reportTime = this.GongYeDateValue.toLocaleString().slice(0, 10).replace(/\//g, '-').trim()
        obj.powerCode = this.powerCode
        obj.deviceCode = this.GongYeValue
        obj.dataItemKey = 'in_tem-in_hum'
        const c = await getSelectEChartsApi(obj)
        if(c.data.length > 0){
          const wenDu = c.data.map(item => {
            return item.in_tem
          })
          const shiDu = c.data.map(item => {
            return item.in_hum
          })
          const time = c.data.map(item => {

<template>
  <el-dialog class="operationDiglog explain-table offlinclass" custom-class="no-scroll" title="离线设备"
    :close-on-click-modal="false" :visible.sync="visibleSync" width="60%" destroy-on-close append-to-body
    :lock-scroll="true" :before-close="closeLogin">
    ...
    <el-row :gutter="24" class="topFormRow">
      ...
      <el-col :span="10" :offset="10">
        <div ref="main" class="grid-content bg-purple">
          <el-input v-model="queryParams.searchValue" placeholder="请输入设备名称、桩号,回车搜索" clearable size="small"
    ...
    <div ref="cc" class="syxt_searchBox" v-show="boxShow">
      <el-form ref="queryForm" :inline="true" :model="queryParams" label-width="75px">
        <el-form-item label="设备方向">
          <el-checkbox-group v-model="checkeBox" @change="handleCheckChange">
            <el-checkbox v-for="item in dict.type.sd_direction" :key="item.value" :label="item.value"
              name="check_direction">{{ item.label }}
            </el-checkbox>
            <el-checkbox name="check_direction" key="3" label="3">双向
        ...
        <el-form-item label="设备类型" prop="eqType">
          ...
          <el-cascader ref="cc1" v-model="queryParams.eqType" :options="eqTypeData" :props="equipmentTypeProps"
            :show-all-levels="false" clearable placeholder="请选择设备类型" @change="changeEquipmentType" style="width: 100%"
            @visible-change="elCascaderOnClick" :key="refresh" popper-class="cascaderClass" :append-to-body="false">
          </el-cascader>
        </el-form-item>
    ...
    <el-table v-loading="loading" :data="devicesList" height="45vh" :row-key="getRowKey" ref="tableFile">

      <el-table-column type="index" :index="indexMethod" label="序号" width="55" align="center"></el-table-column>

      <el-table-column label="所属隧道" align="center" prop="tunnelName.tunnelName" width="160" show-overflow-tooltip />
      <el-table-column label="设备名称" align="center" prop="eqName"  show-overflow-tooltip />
      <el-table-column label="设备类型" align="center" prop="typeName.typeName" width="170" show-overflow-tooltip />

      <el-table-column label="设备方向" align="center" prop="eqDirection" show-overflow-tooltip width="170">
      ...
      <el-table-column label="设备状态" align="center" prop="eqStatus" width="120">
        <template slot-scope="scope">
          <span v-if="scope.row.eqStatus == '1'">在线</span>
          <span v-else-if="scope.row.eqStatus == '2' || !scope.row.eqStatus">离线</span>
    ...
    <pagination v-show="total > 0" :total="total" :page.sync="queryParams.pageNum" :limit.sync="queryParams.pageSize"
      @pagination="getList" style="margin-bottom: 0px !important;" />

  ...
  <div class="app-container">
    ...
    <div class="my-back" :style="{height: 'calc(100vh - ('+ navigationHeight + 'px))'}">
      <div class="header workbench-header">
        <div class="flex-row" style="position: absolute; right: 0px; top: 0.7vh; z-index: 8">
          <div class="display-box zoomClass">
            <p class="zoom-title" style="font-size: 14px;">缩放：</p>
            <el-input-number v-model="zoom" :step="10" :min="60" :max="140" step-strictly @change="zoomChange">
              {{ zoom + '%' }}
            </el-input-number>
          </div>
          ...
          <el-button class="flex-row" type="primary" size="mini" icon="el-icon-files" @click="batchManage">
            批量操作
          </el-button>
          <el-button class="flex-row" type="primary" size="mini" icon="el-icon-top" @click="iconExplain">
            图标含义
          </el-button>
          <el-button class="flex-row" type="primary" size="mini" icon="el-icon-tickets" @click="operationLogPage" style="margin-right: 1vw;">
            操作日志
          </el-button>
        </div>
        <el-row class="menu-b" style="display: flex;align-items: center;" :style="isManagementStation ? 'padding-left:100px;' : ''">
          <el-cascader v-model="tunnelQueryParams.deptId" :options="siteList" :props="siteProps" :show-all-levels="false"
            @change="changeSite" placeholder="请选择" size="mini" class="siteClass" popper-class="popper-class-site"
            v-show="!isManagementStation" />
          <el-button-group class="menu-button-group" style="margin-left: 3px;" :style="isManagementStation ? 'display: flex;':''">
            <el-tooltip class="item" popper-class="wb-tip" v-for="(item, index) in tunnelList" :key="index" effect="dark"
              :content="item.tunnelLength" placement="top-start">
              <el-button type="info" size="mini" @click="setTunnel(item, index)" :class="index ==  buttonIndex?'tunnelBtnStyle':''"
      ...
      <div class="vehicleLane">
        ...
        <div class="content" ref="divRoller" @wheel.prevent="handleTableWheel" @contextmenu.prevent style="position: relative; left: 0px;">
          <!-- <div class="tunnelBox" :style="{ width: currentTunnel.lane.width + 80 + 'px' }" style="border: solid 1px yellow;"> -->

          <div class="workbench-content" @mousedown="dragImg" ref="dragImgDom" @contextmenu.prevent>

            <!--画布区域-->
            <div>
              <el-row class="config-img-area" :style="{ width: currentTunnel.lane.width + 'px' }">
                <el-image class="back-img" :src="currentTunnel.lane.url" :style="{ width: currentTunnel.lane.width + 'px' }"></el-image>

                <div class="wrapper" id="eq-wrapper" @mousedown="dian" @mousemove="yi" @mouseup="li" @mouseover='mouseoversImage'
                  @mouseleave='mouseleaveImage'>
                  <!-- 鼠标移动时产生的蓝框 -->
                  <div id="container" v-if="move == true" :style="{
                  ...
                  <!-- 设备图标-->
                  <div class="icon-box mouseHover" v-for="(item, index) in selectedIconList" :key="item.eqId" :style="{
                    ...
                    <input :class="[
                      ...
                      v-if="(item.display == true &&
                              ...
                              ((item.eqType == 7 ||
                                ...
                                displayNumb == true)
                            "
                      v-show="item.eqType || item.eqType==0" type="text" v-model="item.pile" disabled="true" style="color: #055270;" />
      ...
      <div class="siblings">
        <div class="eqTypeListClass">
          <div type="info" size="small" v-for="(item,index) in dictList" :key="index" :label="item.label" :value="index"
            @click="displayControl(index, item.label)" class="leftButtonS">
            <div>{{ item.label }}</div>
          </div>
        </div>
        <!-- 右侧侧窗 -->
    ...
    <!-- 操作日志 弹窗 -->
    <el-dialog class="workbench-dialog batch-table eventDiglog" :title="title" :visible.sync="operationLogDialog" width="1000px"
      append-to-body v-dialogDrag>
      <el-form :model="queryParams" ref="queryForm" label-width="68px" size="mini">
        <el-form-item label="设备类型" prop="eqTypeId" style="display: inline-block;">
        ...
        <el-form-item label="隧道名称" prop="tunnelId" style="display: inline-block;margin-left: 20px;">
        ...
        <el-form-item label="创建时间" style="display: inline-block;">
          <el-date-picker v-model="dateRange" size="small" style="width: 360px;" value-format="yyyy-MM-dd HH-mm-ss"
            type="datetimerange" range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期" :default-time="['00:00:00', '23:59:59']"
            class="dateRange"></el-date-picker>
        </el-form-item>
        <el-form-item style="display: inline-block;margin-left: 20px;">
          <el-button type="cyan" icon="el-icon-search" size="mini" class="submitButton" @click="handleQuery">搜索</el-button>
          <el-button icon="el-icon-refresh" size="mini" @click="resetQuery" style="color: white;">重置
          </el-button>
        </el-form-item>
      </el-form>
      <el-table v-loading="loading" :data="logList" min-height="200" :default-sort="{prop: 'createTime', order: 'descending'}"
        @selection-change="handleSelectionChange" empty-text="暂无操作日志">
    ...
    <!-- 隧道选择对话框-->
    <el-dialog class="workbench-dialog" :title="title" :visible.sync="tunnelVisible" width="600px" append-to-body>
      <el-row>
        <el-checkbox-group v-model="checkboxTunnel">
          <el-checkbox class="tunnel-checkbox" v-for="(item, index) in tunnelList" :key="index" :label="item" border>
            {{ item.tunnelName }}
          </el-checkbox>
        </el-checkbox-group>
      </el-row>
    ...
    <el-dialog v-dialogDrag class="workbench-dialog batch-table batch-dialog" :title="title" :visible.sync="batchVisible"
      ...
      <el-form ref="batchForm" :rules="rules" :model="batchForm" label-width="80px" label-position="left" size="mini"
        ...
        <template v-if="batchForm.eqType == 1">
          <el-form-item label="车道">
            <el-select v-model="batchForm.eqlane" size="mini">
              <el-option v-if="allLane.length > 0" v-for="item in allLane" :key="item.lane" :label="item.laneName"
                :value="item.lane" />
            </el-select>
          </el-form-item>
        </template>
        <el-table ref="multipleTable" :data="devicesList" tooltip-effect="dark" style="width: 100%" max-height="220"
          size="mini" @selection-change="handleSelectionChange" @row-click="handleRowClick" empty-text="暂无设备">
          <el-table-column type="selection" width="55"></el-table-column>
          <el-table-column prop="eqName" label="设备名称" width="220" show-overflow-tooltip>
          </el-table-column>
          <el-table-column prop="pile" label="桩号" width="120" show-overflow-tooltip>
          </el-table-column>
        ...
        <el-form-item label="配置状态:" prop="state" style="margin-top: 20px">
          <div class="wrap">
            <el-radio-group v-for="(items,index) in eqTypeStateList" :key="index" v-model="batchForm.state" style="display: flex; flex-direction: column">
              <el-radio v-if="batchForm.eqType == items.type && items.control == 1" class="el-radio flex-row" :label="items.state"
                :class="[
                ...
                <el-row v-if="items.url.length > 1 && batchFormDirection == 0">
                  <el-row class="flex-row align-items-center">
                    <img v-for="(url, indexs) in items.url" class="equipment-img" :key="indexs" :src="url" />
                  </el-row>
                  <el-row class="flex-row align-items-center form-direction" style="margin-top: 5px">
                    <el-row>正</el-row>
                    <el-row>反</el-row>
                ...
                <el-row v-else-if="items.url.length > 1 && batchFormDirection == 1">
                  <el-row class="flex-row align-items-center form-direction">
      ...
      <div slot="footer">
        <el-button type="primary" size="mini" @click="submitBatchState('batchForm')" class="submitButton">确 定
        </el-button>
        <el-button type="primary" size="mini" @click="cancel">取 消</el-button>
      </div>
    </el-dialog>
    ...
    <!--微波车检对话框-->
    <el-dialog v-dialogDrag class="workbench-dialog batch-table" :title="title" :visible.sync="weiboVisible" width="45vw"
      append-to-body>
      <el-table ref="multipleTable" :data="weiboList" tooltip-effect="dark" style="width: 100%" size="mini" empty-text="暂无数据">
        <el-table-column label="车道" align="center" prop="byLane" width="200">
          <template slot-scope="scope">
            <span>第{{ scope.row.byLane }}车道</span>
          </template>
        </el-table-column>
        <el-table-column label="车流量(辆/分钟)" align="center" prop="fSpaceOccupyRation">
          <template slot-scope="scope">
   ...
   <com-video style="position: absolute;top: 0;left: 0;" v-if="this.clickEqType == 23" 
    ...
    <!--道路结冰对话框-->
    <el-dialog v-dialogDrag class="workbench-dialog batch-table" :title="title" :visible.sync="daolujiebingVisible"
      width="40vw" append-to-body style="left: 30%">
    ...
    <!-- 路面状态 -->
    <el-dialog v-dialogDrag class="workbench-dialog batch-table" :title="title" :visible.sync="lumianVisible" width="40vw"
      append-to-body style="left: 30%">
      <el-form ref="form" :model="stateForm" label-position="left" size="mini">
        <el-row>
          <el-col :span="8">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="采集时间:">
              <template slot-scope="scope">
                <span> {{ parseTime(lumianList.timeCollect, "{yyyy}-{m}-{d} {h}:{i}:{s}") }}</span>
              </template>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <el-dialog v-dialogDrag class="workbench-dialog explain-table icon-dialog" :title="title" :visible.sync="explainVisible"
      ...
      <el-table ref="multipleTable" :data="eqIcon" tooltip-effect="dark" style="width: 100%" :max-height="400" size="mini"
        @selection-change="handleSelectionChange" empty-text="暂无设备">
        <el-table-column label="图标">
          <template slot-scope="scope">
            　 <img :src="scope.row.url" style="vertical-align: middle;max-height: 30px;" />
          </template>
        </el-table-column>
        <el-table-column prop="type" label="所属类型" width="120" show-overflow-tooltip>
        </el-table-column>
    ...
    <el-dialog v-dialogDrag class="workbench-dialog explain-table strategyClass eventDiglog" :title="title"
      ...
      <el-table ref="multipleTable" :data="strategyList" tooltip-effect="dark" style="width: 100%" :max-height="400"
        size="mini" @selection-change="handleSelectionChange" :row-class-name="tableRowClassName" empty-text="暂无策略">
        ...
        <el-table-column label="策略信息" align="center" prop="strategyInfo">
          <template slot-scope="scope" v-if="scope.row.slist!=[]">
            <div v-for="(item,index) in scope.row.slist" :key="index">{{ item }}</div>
          </template>
          <div v-else>暂无信息</div>
        </el-table-column>
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button v-show="scope.row.strategyType == 0" size="mini" type="text" icon="el-icon-thumb" @click="handleController(scope.row)">手动控制
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 火灾报警 对话框 -->
    <el-dialog title="火灾报警提示" :visible.sync="HZdialogVisible" width="25%" :before-close="HZhandleClose" class="temperatureDialog">
      <div slot="title">
    ...
    <!-- 限高控制 对话框 -->
    <el-dialog title="限高杆控制" :visible.sync="controlHeightVisible" :before-close="controlHeightClose" class="workbench-dialog"
      v-dialogDrag width="560px" append-to-body>
      <div>
        <span>隧道名称：</span>
      ...
      <div>
        <el-progress :text-inside="true" :stroke-width="26" :percentage="percentage" :color="customColors" style="width: 60%;display: inline-block;"></el-progress>
        <div style="display: inline-block;margin-left: 20px;color: white !important;">
          <el-button-group>
    ...
    <!--隧道诱导对话框-->
    <el-dialog v-dialogDrag class="workbench-dialog batch-table youdaoDialog" :title="title" :visible.sync="youdaoVisible"
      width="40vw" append-to-body>
      <el-form ref="form" :model="stateForm" label-width="80px" label-position="left" size="mini">
        <el-row>
          <!-- <el-col :span="8">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="控制模式:" prop="corModel">
              <el-radio-group v-model="stateForm.corModel" @change="outLight">
                <el-col :span="4">
                  <el-radio key="00" label="00" style="padding: 5px 20px 5px 10px;">闪烁</el-radio>
                </el-col>
                <el-col :span="4">
                  <el-radio key="02" label="02" style="padding: 5px 20px 5px 10px;margin-left: 25px;">常亮</el-radio>
                </el-col>
                <el-col :span="4">
                  <el-radio key="03" label="03" style="padding: 5px 20px 5px 10px;margin-left: 50px;">常灭</el-radio>
                </el-col>
              </el-radio-group>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="车道:" prop="lane">
              <el-checkbox v-model="stateForm.Zlane" label="D" key="D">左车道</el-checkbox>
              <el-checkbox v-model="stateForm.Ylane" label="E" key="E">右车道</el-checkbox>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="白灯亮度:">
              <el-slider v-model="stateForm.whiteLight" :step="10" style="width: 90%;">
              </el-slider>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="黄灯亮度:">
              <el-slider v-model="stateForm.yellowLight" :step="10" style="width: 90%;">
              </el-slider>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row v-if="stateForm.corModel=='00'">
          <el-col :span="12">
            <el-form-item label="闪烁模式:">
              <el-select v-model="stateForm.twinkleModel" placeholder="请选择" style="width: 60%;">
                <el-option v-for="(item,index) in twinkleModelOptions" :key="index" :label="item.label" :value="item.value">
                </el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="闪烁频率:">
              <el-select v-model="stateForm.twinkleFrequency" placeholder="请选择" style="width: 60%;">
                <el-option v-for="(item,index) in twinkleFrequencyOptions" :key="index" :label="item.label" :value="item.value">
                </el-option>
              </el-select>
              次/分钟
            </el-form-item>
        ...
        <el-row v-if="stateForm.corModel=='00'">
          <el-col :span="12">
            <el-form-item label="亮灯时长:">
              <el-select v-model="stateForm.lightTime" placeholder="请选择" style="width: 60%;">
                <el-option v-for="(item,index) in lightTimeOptions" :key="index" :label="item.label" :value="item.value">
                </el-option>
              </el-select>
              毫秒
            </el-form-item>
      ...
      <div slot="footer">
        <el-button type="primary" size="mini" @click="submitCorLight">确 定</el-button>
        <el-button type="primary" size="mini" @click="cancel">取 消</el-button>
      </div>
    </el-dialog>
  </div>
...
<script>
  import {displayH5sVideoAll,stopVideoAll} from  "@/api/icyH5stream";
    ...
    methods: {
      ...
      trafficFlowLane() {
        trafficFlowInformation().then(response => {
          ...
          for (var item of response.data) {
            console.log(item.left, "left", item.width, "width", item.color, "color", item.tunnelId, item.type)
          }
        })
      },
      /** 设备类型 */
          ...
          that.siteList.filter(item => {
            if (item.children) {
              item.children.forEach(val => {
                iid ? '' : iid = val.deptId
                that.tunnelQueryParams.deptId == iid ? '' : that.tunnelQueryParams.deptId = iid
              })
            } else {
              iid ? '' : iid = item.deptId
              that.tunnelQueryParams.deptId == iid ? '' : that.tunnelQueryParams.deptId = iid
      ...
      robotTabsClick(tab, event) {
        ...
        if (tab.paneName == 'stateRecords') {
          this.robotChartDom || (this.robotChartDom = echarts.init(document.getElementById('robotEchart')));
          that.$nextTick(() => {
          ...
          tooltip: {
            ...
            formatter: function(value) {
              for (var j = 0; j < value.length; j++) {
                if (value[j].value == 0) {
                  return value[j].name + '<br/> ' + value[j].seriesName + ": 正常";
                } else if (value[j].value == 1) {
                  return value[j].name + '<br/> ' + value[j].seriesName + ": 离线";
                } else if (value[j].value == 2) {
                  return value[j].name + '<br/> ' + value[j].seriesName + ": 故障";
                }
              }
            },
            textStyle: {
      ...
      initEnergyConsumption(sevenDaysBefore) {
        ...
        sevenDaysBefore[8] = this.dateFormat(nowMs - 1000 * 60 * 60 * 24 * parseInt(1)); //前几天，n就取几，整数
        sevenDaysBefore[7] = this.dateFormat(nowMs - 1000 * 60 * 60 * 24 * parseInt(3)); //前几天，n就取几，整数
        sevenDaysBefore[6] = this.dateFormat(nowMs - 1000 * 60 * 60 * 24 * parseInt(2)); //前几天，n就取几，整数
        sevenDaysBefore[5] = this.dateFormat(nowMs - 1000 * 60 * 60 * 24 * parseInt(4)); //前几天，n就取几，整数
        sevenDaysBefore[4] = this.dateFormat(nowMs - 1000 * 60 * 60 * 24 * parseInt(5)); //前几天，n就取几，整数
        sevenDaysBefore[3] = this.dateFormat(nowMs - 1000 * 60 * 60 * 24 * parseInt(6)); //前几天，n就取几，整数
        sevenDaysBefore[2] = this.dateFormat(nowMs - 1000 * 60 * 60 * 24 * parseInt(7)); //前几天，n就取几，整数
        sevenDaysBefore[1] = this.dateFormat(nowMs - 1000 * 60 * 60 * 24 * parseInt(8)); //前几天，n就取几，整数
        sevenDaysBefore[0] = this.dateFormat(nowMs - 1000 * 60 * 60 * 24 * parseInt(9)); //前几天，n就取几，整数
        ...
        var safetyArr = [1450, 1650, 1500, 1430, 1580, 1530, 1580, 1460, 1400, 1540];
        var option = {
        ...
        this.timer = setInterval(() => {
          if (parseInt(car.style.left) <= 30) {
            ...
            for (let i = 0; i < this.selectedIconList.length; i++) {
              ...
              if ((item.eqType == 7 && item.eqDirection == 1) || (item.eqType == 8 && item.eqDirection == 1) || (
      ...
      carMapLeft() {
        ...
        this.timer = setInterval(() => {
          if (parseInt(car.style.left) >= 1130) {
            ...
            for (let i = 0; i < this.selectedIconList.length; i++) {
              let item = this.selectedIconList[i];
              if (item.eqType == 7 && item.eqDirection == 0 || item.eqType == 8 && item.eqDirection == 0 || item.eqType ==
                9 && item.eqDirection == 0) {
                item.url[0] = this.dark;
              }
            }
          } else {
      ...
      li() {
        ...
        if (boxEqList.length > 0 && this.boxTypeList.length > 0) {
          if (boxEqList.length == 1 && boxEqList[0].eqlist.length == 1) {
            ...
            console.log(boxEqList[0].eqlist[0],'boxEqList[0].eqlist[0]boxEqList[0].eqlist[0]')
      ...
      displayControl(value, lable) {
        ...
        $('.leftButtonS').eq(value).addClass('leftButton_hover').siblings().removeClass('leftButton_hover')
          ...
          if (typelist.length > 0) {
            for (let y = 0; y < typelist.length; y++) {
              for (let i = 0; i < this.selectedIconList.length; i++) {
                ...
                if (this.selectedIconList[i].eqType == null || this.selectedIconList[i].eqType == undefined) {
      ...
      async openStateSwitch(item) {
        await getDevices(item.eqId).then(res=>{
        ...
        //跳转 摄像机
        else if (item.eqType == "24" || item.eqType == "23" || item.eqType == "25") {
          console.log("点击摄像机")
          getDevices(item.eqId).then(res=>{
            displayH5sVideoAll(res.data.secureKey);
          })
          // getSecureKey().then(token=>{
          //   console.log(token,'&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
          //   displayH5sVideoAll(token);
          // })
          // this.stateForm = {
          //   eqId: item.eqId,
          //   eqHostId: item.eqHostId,
      ...
      submitCorLight() {
        ...
        this.setCorLight(param).then(response => {
      ...
      setCorLight(param) {
        setCorLight(param).then(response => {

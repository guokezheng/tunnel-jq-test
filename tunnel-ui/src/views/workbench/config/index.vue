  ...
  <div class="app-container">
    <div class="my-back">
      ...
      <div class="header workbench-header">
        <el-row class="menu-b" style="display: flex; align-items: center; z-index: 8"
          :style="isManagementStation ? 'padding-left:100px;' : ''">
          <el-cascader v-model="tunnelQueryParams.deptId" :options="siteList" :props="siteProps"
            @visible-change="cascaderHandleChange" @expand-change="cascaderHandleChange" :show-all-levels="false"
            @change="changeSite" placeholder="请选择" size="mini"
            :class="manageStation == '1' ? 'siteClassDisabled' : 'siteClass'" popper-class="popper-class-site"
          ...
          <el-button-group class="menu-button-group" style="margin-left: 3px"
            ...
            <el-tooltip class="item" popper-class="wb-tip" v-for="(item, index) in tunnelList" :key="item.tunnelId"
              effect="dark" placement="top-start">
              <div slot="content" v-html="splitData(item.tunnelLength)"></div>
              <el-button type="info" size="mini" @click="setTunnel(item, index)"
                :class="index == buttonIndex ? 'tunnelBtnStyle' : ''" style="
                  display: flex;
        ...
        <div class="flex-row buttonsBox">
          ...
          <el-button v-if="resetCanvasFlag" class="flex-row" type="primary" size="mini" icon="el-icon-map-location"
            @click="resetCanvas">
            地图复位
          </el-button>
          <el-button class="buttons" type="primary" size="mini" @click="strategyPage">
          ...
          <!--            <span>控制策略1</span>-->
          <!--          </el-button>-->
          <el-button class="buttons" type="primary" size="mini" @click="strategyPage2">
            <img src="../../../assets/icons/kzcl.png" />
            <span>智慧调光</span>
          </el-button>
        ...
        <el-row>
          ...
          <el-col :span="12">
            <div class="drawerItem zoomClass">
              ...
              <el-switch v-model="carShow" class="switchStyle" @change="carShowChange"></el-switch>
            </div>
          </el-col>
          <el-col :span="12">
            <div class="drawerItem zoomClass">
      ...
      <div class="vehicleLane" @mouseover="mouseoversImage" @mouseleave="mouseleaveImage">
        <div class="content" ref="divRoller" @wheel.prevent="handleTableWheel" @mousewheel.prevent="mouseSrollAuto"
          ...
          <div class="workbench-content" ref="dragImgDom">
            <!--画布区域-->
            <div>
              <el-row class="config-img-area" id="config-img-id" :style="{ width: currentTunnel.lane.width + 'px' }">
                <el-image class="back-img" :src="currentTunnel.lane.url"
                  :style="{ width: currentTunnel.lane.width + 'px' }"></el-image>
                ...
                > -->
                  ...
                  <!-- 测控执行器 -->
                  <div class="actuatorCK" v-show="
                      selectBigType.index == 0 &&
                      selectedIconList.length > 0 &&
                      currentTunnel.id == 'JQ-JiNan-WenZuBei-MJY'
                    " @click="openStateSwitch(actuatorCKItem)" @mousemove="openTooltip2()"
                  ...
                  <div class="actuatorCKTooltipBox" v-if="actuatorTooltip">
                    <span>名称：{{ actuatorCKItem.eqName }}</span><br />
                  ...
                  <div class="accident" v-for="(itme, index) in carActionExplodeList" :key="index">
                    <!--                  <svg :style="{-->
                    <!--                      right: itme.right,-->
                    <!--                      left: itme.left,-->
                    <!--                      top: itme.top,-->
                    ...
                    <div v-if="itme.pictureUrl == '1'">
                      <el-image class="back-img my-div" :src="require('@/assets/image/accident/爆炸.svg')" fit="none"
                        :style="{
                          right: itme.right,
                          left: itme.left,
                          top: itme.top,
                          width: 48 + 'px',
                          height: 48 + 'px',
                        }" @click="CatImageClick(itme)"></el-image>
                    </div>
                    <div v-if="itme.pictureUrl == '2'">
                      <el-image class="back-img my-div" :src="require('@/assets/image/accident/hz.svg')" fit="none"
                        :style="{
                          right: itme.right,
                          left: itme.left,
                          top: itme.top,
                  ...
                  <div class="carBox" v-show="carShow" v-for="(value, key) in carList" :key="key">
                    <div id="carShowSpan" v-for="(data, index) in value" :key="index" :style="{
                        right: data.right,
                        left: data.left,
                        top: data.top,
                      }">
                      <el-tooltip disabled :title="data.vehicleLicense" placement="bottom" effect="light">
                        <span class="my-div" :title="data.vehicleLicense" :style="{ color: data.background }"
                  ...
                  <div class="icon-box mouseHover" v-for="(item, index) in selectedIconList" :key="item.eqId" :style="{
                    ...
                    <div v-show="
                      ...
                      <!-- CO/VI -->
                      <label style="color: #79e0a9" class="labelClass" v-if="item.eqType == 19">
                        {{ item.num }}
                      </label>
                      <!-- 风速风向 -->
                      <label style="color: #79e0a9" class="labelClass" v-if="item.eqType == 17">
                        {{ item.num }}
                      </label>
                      <!-- 洞内洞外 -->
                      <label style="color: #f2a520" class="labelClass" v-if="item.eqType == 5 || item.eqType == 18">
                        {{ item.num }}
                      </label>
                      <!-- 加强照明 -->
                      <label style="color: #f2a520" class="labelClass labelClass7" v-if="item.eqType == 7">
                        <span v-if="
                    ...
                    <input :class="[
                        ...
                        ((item.eqType == 7 ||
                          ...
                          lightSwitch == 1 &&
                          displayNumb == true)
                      " v-show="item.eqType || item.eqType == 0" type="text" v-model="item.pile" disabled="true"
    ...
    <el-dialog class="workbench-dialog vehicle-dialog" :title="title" :visible.sync="batchManageDialog" width="450px"
      ...
      <el-table ref="batchManageTable" :data="batchManageList" tooltip-effect="dark"
        style="width: 100%; margin-bottom: 10px !important" max-height="220" size="mini" @row-click="handleRowClick">
    ...
    <!-- 操作日志 弹窗 -->
    <el-dialog class="explain-table operationDiglog" :title="title" :close-on-click-modal="false"
      :visible.sync="operationLogDialog" :before-close="cancel" width="1100px" :destroy-on-close="true" append-to-body
      v-dialogDrag @close="resetQuery">
      <div class="dialogStyleBox">
        <div class="dialogLine"></div>
        <div class="dialogCloseButton"></div>
      </div>
      ...
      <el-row :gutter="20" style="margin: 0px 0 10px" v-show="operationActive == 'xitong'">
        <el-col :span="4">
          <el-button size="small" @click="resetQuery">刷新</el-button>
        </el-col>
        <el-col :span="10" :offset="10">
          <div class="grid-content bg-purple" ref="main">
            <el-input placeholder="请输入用户名称、登录地址，回车搜索" v-model="operationParam_xt.ipaddr"
              @keyup.enter.native="handleQueryOperationParam" size="small">
              <el-button slot="append" class="searchTable" @click="syxt_boxShow = !syxt_boxShow"></el-button>
            </el-input>
          </div>
        </el-col>
      </el-row>
      <div class="syxt_searchBox" v-show="syxt_boxShow" ref="cc">
        <el-form ref="operationParam_xt" :inline="true" :model="operationParam_xt" label-width="68px"
          v-show="operationActive == 'xitong'">
          <el-form-item label="登录状态" prop="status">
            <el-select v-model="operationParam_xt.status" clearable placeholder="请选择登录状态" size="small"
              style="width: 100%">
          ...
          <el-form-item label="登录时间">
            <el-date-picker v-model="dateRange" size="small" style="width: 100%" value-format="yyyy-MM-dd HH:mm:ss"
              ...
              :picker-options="setoptions" :default-time="['00:00:00', '23:59:59']"
              :class="this.sideTheme != 'theme-dark' ? 'themeDarkPicker' : ''"></el-date-picker>
          </el-form-item>
          <el-form-item class="bottomBox">
            <el-button size="small" type="primary" @click="handleQueryOperationParam">搜索</el-button>
            <el-button size="small" @click="resetQuery" type="primary" plain>重置</el-button>
          </el-form-item>
        </el-form>
      </div>

      <el-row :gutter="20" style="margin: 0px 0 10px" v-show="operationActive == 'caozuo'">
        <el-col :span="4">
          <el-button size="small" @click="resetQuery">刷新</el-button>
        </el-col>
        <el-col :span="10" :offset="10">
          <div class="grid-content bg-purple" ref="main1">
            <el-input placeholder="请输入设备名称、桩号、操作地址，回车搜索" v-model="operationParam.operIp"
              @keyup.enter.native="handleQueryOperationParam" size="small">
              <el-button slot="append" class="searchTable" @click="sycz_boxShow1 = !sycz_boxShow1"></el-button>
            </el-input>
          </div>
        </el-col>
      </el-row>
      <div class="syxt_searchBox" v-show="sycz_boxShow1" ref="cc1">
        <el-form ref="operationParam" :inline="true" :model="operationParam" label-width="68px"
          ...
          <el-form-item label="控制方式" prop="controlType" style="width: 100%">
            <el-select v-model="operationParam.controlType" clearable placeholder="请选择控制方式" size="small">
              <el-option v-for="dict in dict.type.sd_control_type" :key="dict.value" :label="dict.label"
                :value="dict.value" />
            </el-select>
          </el-form-item>
          <el-form-item label="创建时间">
            <el-date-picker v-model="dateRange1" size="small" style="width: 100%" value-format="yyyy-MM-dd HH:mm:ss"
              type="datetimerange" range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期"
              :picker-options="setoptions" :default-time="['00:00:00', '23:59:59']"></el-date-picker>
          </el-form-item>
          <el-form-item class="bottomBox">
            <el-button size="small" type="primary" @click="handleQueryOperationParam">搜索</el-button>
            <el-button size="small" @click="resetQuery" type="primary" plain>重置</el-button>
      ...
      <el-table ref="tables" v-loading="loading" :data="operationList1" @selection-change="handleSelectionChange"
        ...
        :default-sort="{ prop: 'loginTime', order: 'descending' }" max-height="430" :row-key="getRowKey1">
        ...
        <el-table-column label="登录地点" align="center" width="90" prop="loginLocation" :show-overflow-tooltip="true" />
        ...
        <el-table-column label="登录日期" align="center" prop="loginTime" sortable width="170">
          <template slot-scope="scope">
            <span>{{ parseTime(scope.row.loginTime) }}</span>
          </template>
        </el-table-column>
      </el-table>
      <el-table v-loading="loading" :data="operationList2" max-height="430"
        :default-sort="{ prop: 'createTime', order: 'descending' }" @selection-change="handleSelectionChange"
        :row-class-name="tableRowClassName" v-show="operationActive == 'caozuo'" :row-key="getRowKey2">
        <el-table-column type="index" :index="indexMethod2" label="序号" width="50" align="center"></el-table-column>
        <el-table-column label="隧道名称" align="center" prop="tunnelName.tunnelName" width="90" />
        <el-table-column label="设备类型" align="center" prop="typeName.typeName" :show-overflow-tooltip="true" />
        <el-table-column label="设备名称" align="center" prop="eqName.eqName" width="130" :show-overflow-tooltip="true" />
        <el-table-column label="桩号" align="center" prop="pile" width="120" />
        <el-table-column label="操作状态" align="center" prop="stateName.stateName" width="130"
          :show-overflow-tooltip="true" />
        <el-table-column label="控制方式" align="center" prop="controlType" width="90" :show-overflow-tooltip="true"
          :formatter="controlTypeFormat" />
        <el-table-column label="操作结果" align="center" prop="state" width="80" />
      ...
      <pagination v-show="total1 > 0 && operationActive == 'xitong'" :total="total1"
        :page.sync="operationParam_xt.pageNum" :limit.sync="operationParam_xt.pageSize"
        @pagination="getOperationList(operationActive)" class="paginationWorkbench" />
      <pagination v-show="total2 > 0 && operationActive == 'caozuo'" :total="total2" :page.sync="operationParam.pageNum"
        :limit.sync="operationParam.pageSize" @pagination="getOperationList(operationActive)"
    ...
    <!-- 照明配置对话框 -->
    <el-dialog v-dialogDrag class="workbench-dialog explain-table icon-dialog" :title="title"
      :visible.sync="lightingVisible" width="45%" append-to-body :close-on-click-modal="false">
      <div class="dialogStyleBox">
        <div class="dialogLine"></div>
        <div class="dialogCloseButton"></div>
      </div>
      <el-form ref="form" :model="lightingForm" label-width="120px" :rules="lightingFormRules">
          ...
          <el-col :span="12">
            <el-form-item label="模式类型" prop="modeType">
              <el-select v-model="lightingForm.modeType" placeholder="请选择模式类型">
                <el-option v-for="item in modeTypeList" :key="item.value" :label="item.name" :value="item.value" />
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="开启" prop="isStatus" v-if="lightingForm.modeType != 0">
              <el-tooltip :content="lightingForm.isStatus == 1 ? '开启' : '关闭'" placement="top">
                <el-switch v-model="lightingForm.isStatus" active-color="#13ce66" inactive-color="#ff4949"
    ...
    <!--查看控制策略对话框-->
    <el-dialog v-dialogDrag class="explain-table operationDiglog" :title="title" :visible.sync="strategyVisible"
      width="1000px" append-to-body :close-on-click-modal="false" :destroy-on-close="true" @close="strategyResetQuery">
      <div class="dialogStyleBox">
        <div class="dialogLine"></div>
        <div class="dialogCloseButton"></div>
      </div>
      ...
      <el-row :gutter="20" style="margin: 0px 0 10px" v-show="
        ...
        <el-col :span="10" :offset="10">
          <div class="grid-content bg-purple" ref="main2">
            <el-input v-model="queryParams.strategyName" placeholder="请输入策略名称" @keyup.enter.native="handlestrategyQuery"
              size="small">
              <el-button slot="append" class="searchTable" @click="syxt_boxShow2 = !syxt_boxShow2"></el-button>
            </el-input>
          </div>
        </el-col>
      </el-row>
      <div class="syxt_searchBox" v-show="syxt_boxShow2" ref="cc2">
        <el-form ref="operationParam" :inline="true" :model="operationParam" label-width="68px">
          ...
          <el-form-item class="bottomBox">
            <el-button size="small" @click="handlestrategyQuery">搜索</el-button>
      ...
      <el-table ref="multipleTable" :data="strategyList" tooltip-effect="dark" v-loading="strategyLoading"
        style="width: 100%" :max-height="420" size="mini" @selection-change="handleSelectionChange"
        :row-class-name="tableRowClassName" empty-text="暂无策略" :key="tableKey">
        ...
        <el-table-column label="状态" align="center" prop="schedulerTime" v-if="strategyActive != 'richang'">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.strategyState" active-color="#39ADFF" inactive-color="#ccc" active-value="0"
              inactive-value="1" v-hasPermi="['workbench:dialog:save']" @change="changeStrategyState(scope.row)">
            </el-switch>
          </template>
        </el-table-column>
        <el-table-column label="执行" align="center" prop="schedulerTime" v-if="strategyActive == 'richang'">
          <template slot-scope="scope">
            <el-button size="mini" type="text" class="tableBlueButtton" v-hasPermi="['workbench:dialog:save']"
              @click="richanghandleUpdate(scope.row)">执行</el-button>
          </template>
        </el-table-column>
      </el-table>
      <pagination v-show="total > 0" :total="total" :page.sync="queryParams.pageNum" :limit.sync="queryParams.pageSize"
        @pagination="handlestrategyQuery" class="paginationWorkbench" />
      <div style="clear: both;"></div>
    </el-dialog>
...
<script>
  ...
  import {
    singleCat
  } from "@/views/workbench/config/mixin/single-cat";
    ...
    methods: {
      ...
      batchManageOK() {
        ...
        if (this.itemEqType == 9 && this.batchManageForm.brightness < 30 && this.batchManageForm.state == 1) {
          ...
          return;
        } else if ((this.itemEqType == 9 && !this.batchManageForm.brightness) ||
      ...
      async openStateSwitch(item) {
        ...
        if (this.addBatchManage == true) {
          ...
          if (item.isControl == "1" && ![16, 22, 36, 29].includes(item.eqType)) {
            ...
            item.isControl == "0" || [16, 22, 29, 33, 36].includes(item.eqType)
          ) {

...
/**
 *  多连接客户端 --- 其中 synchronized(channel) 可以采用 单任务队列的方式
...
public class MultClient {
    ...
    public void init(int count){
        ...
        for(int i=1; i<=count; i++){
            ChannelFuture future = bootstrap.connect("192.168.0.103", 9008);
    ...
    private void reconnect(Channel channel){
        synchronized(channel){
            ...
            Channel newChannel = bootstrap.connect("192.168.0.103", 9008).channel();
